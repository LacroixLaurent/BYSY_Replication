...
#	geom_text_repel(data=featROI%>% filter(type %in% c("ORI")),aes(x=center,y=2.4,label=featname,col=ars_col),size=1,angle=45,hjust = 0,show.legend=F,direction="x",seed=12,ylim=c(2,4),min.segment.length=10)+
	geom_text(data=featROI%>% filter(type %in% c("ORI")),aes(x=center,y=2.4,label=featname,color=ars_col),size=1.7,angle=45,hjust = 0,show.legend=F)+
...
pl <- ggplot(toplot_nano2)+
	...
	geom_text(data=featROI2,aes(x=start,y=1.95),label="*",size=6)+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=start),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=end),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("chromBY")),aes(xintercept=start),linetype=4,alpha=0.8)+
	geom_vline(data=featROI%>% filter(type %in% c("chromBY")),aes(xintercept=end),linetype=4,alpha=0.8)+
...
p_list[[i]] <- pl_rfdpooled/pl_initpooled/pl_terpooled/pl_nanopooled/plot_spacer()/pl_gen+plot_layout(ncol = 1, heights = c(rep(3,4),1.6,2)) & themeadj
}
...
ggsave(plot=pp,paste0("FigArticle/Fig4.pdf"),h=9.05,w=6.88,device=cairo_pdf)
#quartz(paste0("FigArticle/Fig4.pdf"),h=9.05,w=6.88)
#print(pp)
#dev.off
...
ggsave(plot=pp,paste0("FigArticle/FigS4AD.pdf"),h=9.05,w=6.88,device=cairo_pdf)
...
ggsave(plot=pp,paste0("FigArticle/FigS4EH.pdf"),h=9.05,w=6.88,device=cairo_pdf)
...
ggsave(plot=pp,paste0("FigArticle/FigS4IL.pdf"),h=9.05,w=6.88,device=cairo_pdf)
```
