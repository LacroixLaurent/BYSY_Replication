...
ggplot(featROI)+
	geom_rect(data=featROI %>% filter(type %in% c("CEN","rDNA","Ty_masked","ura3_masked")),aes(xmin=start,xmax=end,ymin=2,ymax=3,col=type,fill=type),linewidth=0.2)+
	geom_rect(data=featROI%>% filter(type %in% c("ORI")),aes(xmin=start,xmax=end,ymin=1,ymax=2,col=type,fill=type),linewidth=0.2)+
	geom_rect(data=featROI%>% filter(type %in% c("chromBY")),aes(xmin=pmax(start,start(ROI)),xmax=pmin(end,end(ROI)),ymin=0,ymax=1,col=featname,fill=featname),linewidth=0.2)+
	geom_text(data=featROI%>% filter(type %in% c("chromBY")),aes(x=(pmax(start,start(ROI))+pmin(end,end(ROI)))/2,y=0.5,label=featname),col="black",show.legend=F,size=1)+
	...
	scale_x_continuous(
		...
		expand=c(0,0))+
	theme(axis.ticks.y = element_blank(),axis.text.y = element_blank(),panel.grid.major.y=element_blank(),panel.grid.minor.y=element_blank(),legend.key = element_rect(colour = "black",linewidth=0.2))
})
...
p1 <- ggplot(toplot_bin2)+
	geom_line(aes(x=pos,y=BYspeed,col="speedBY"),linewidth=0.25)+
	geom_ribbon(aes(x=pos,ymin=BYspeedINF,ymax=BYspeedSUP,fill="speedBY"),alpha=0.3,col=NA)+
	geom_line(aes(x=pos,y=SYspeed,col="speedSY"),linewidth=0.25)+
	geom_ribbon(aes(x=pos,ymin=SYspeedINF,ymax=SYspeedSUP,fill="speedSY"),alpha=0.3,col=NA)+
	geom_hline(yintercept=speedmedgenBY,linetype=2,alpha=0.5,col=speed_pal[1],linewidth=0.2)+
	geom_hline(yintercept=speedmedgenSY,linetype=2,alpha=0.5,col=speed_pal[2],linewidth=0.2)+
	...
	theme(legend.key = element_rect(colour = "black",linewidth=0.2))+
...
p2 <- ggplot(toplot_wt2)+geom_tile(aes(x=pos+500,y=type2,fill=wt2))+
	...
	scale_x_continuous(
		...
		minor_breaks=seq(floor(start(ROI)/50000)*50000,end(ROI),50000),
		expand=c(0,0))+
	theme(axis.ticks.y = element_blank(),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),legend.key = element_rect(colour = "black",linewidth=0.2))
...
themeadj <- theme(
	text = element_text(size =6),  # Taille des polices
...
pl_gen <- pl_geno[[i]]+theme(legend.title=element_blank(),legend.key.size = unit(0.05, 'cm'))+ylab("Features")
pl_testSYBY <- pl_MWW_SYBY[[i]]+theme(legend.title=element_blank(),axis.title.x=element_blank(),legend.key.size = unit(0.05, 'cm'))+ylab("Scale_BYvsSY")
pl_testBY <- pl_MWW_BY[[i]]+theme(legend.title=element_blank(),axis.title.x=element_blank(),legend.key.size = unit(0.05, 'cm'))+ylab("Scale_BY")
pl_testSY <- pl_MWW_SY[[i]]+theme(legend.title=element_blank(),axis.title.x=element_blank(),legend.key.size = unit(0.05, 'cm'))+ylab("Scale_SY")
p_list[[i]] <- pl_speed/pl_testSYBY/pl_testBY/pl_testSY/pl_gen+plot_layout(ncol = 1, heights = c(3.8,1.4,1.4,1.4,1)) & themeadj
}
...
ggsave(plot=pp,paste0("FigArticle/Fig7.pdf"),h=6,w=6.8)
...
ggsave(plot=pp,paste0("FigArticle/FigS7AD.pdf"),h=6,w=6.8)
...
ggsave(plot=pp,paste0("FigArticle/FigS7EH.pdf"),h=6,w=6.8)
...
ggsave(plot=pp,paste0("FigArticle/FigS7IL.pdf"),h=6,w=6.8)

```

### FigS10: speed distri vs genomic features
...
fs10a <- ggplot(toplot0,aes(x=feat,y=speed,fill=strain,group=strain))+
	stat_slab(col=NA,alpha=0.6,scale=0.7,position="dodge")+
	stat_pointinterval(
		point_size = 1,
		interval_size_domain = c(0.5, 0.95),
		interval_size_range = c(0.16, 0.21),
    col="grey40",
		position="dodge",
		show.legend=F)+
	geom_text(data=totext,aes(x=feat,y=0,label=n),fontface="italic",position =position_dodge(width=1),size=1.7) +
	geom_text(data=tomed,aes(x=feat,y=4800,label=med),col="red",position =position_dodge(width=1),size=1.7) +
...
fs10b <-  ggplot(toplot,aes(x=feat_name,y=speed,fill=strain,group=strain))+
	stat_slab(col=NA,alpha=0.6,scale=0.7,position="dodge")+
	stat_pointinterval(
		point_size = 1,
		interval_size_domain = c(0.5, 0.95),
    ...
    col="grey40",
		position="dodge",
		show.legend=F)+
	geom_text(data=totext,aes(x=feat_name,y=0,label=n),fontface="italic",position =position_dodge(width=1),size=1.7) +
	geom_text(data=tomed,aes(x=feat_name,y=4800,label=med),col="red",position =position_dodge(width=1),size=1.7) +
	scale_fill_manual("Strain",values=mypal[c(1,7)])+
	coord_cartesian(ylim=c(0,5000))+
	theme(axis.title.x= element_blank(),axis.ticks.x= element_blank(),plot.tag=element_text(face="bold"))+
	ylab("Speed (bp/min)")
...
design <- "
 ...
 22222222222222
"
pl <- fs10a+fs10b+plot_layout(guides="collect",design=design)+plot_annotation(tag_levels="A",title="Supplementary Figure 10",theme=theme(plot.title = element_text(face="bold"))) & theme(plot.tag = element_text(face="bold")) & themeadj

ggsave(plot=pl,paste0("FigArticle/FigS10.pdf"),h=3.4,w=6.8)
