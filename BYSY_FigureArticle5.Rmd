...
### Figure 5 : RFD, RFD_BY, Init,Ter,NanoT on Junc15-11,Junc13-12, Junc 2-3 and 6-14
#### Figure S5: RFD, Init,Ter,NanoT for the others Junc/Tal
...
ars2plot <- inner_join(as_tibble(ARS_BYonSY),MasterTable,by = join_by(name,start,end)) %>%
...
pl_geno <- lapply(seq_along(chromJunction), function(i) {
	ROI <- suppressWarnings(trim(chromJunction[i]+ovl))
...
#	geom_text_repel(data=featROI%>% filter(type %in% c("ORI")),aes(x=start,y=4.4,label=featname,col=ars_col),size=1,angle=45,hjust = 0,show.legend=F,direction="x",seed=12,ylim=c(4,8),min.segment.length=5)+
	geom_text(data=featROI%>% filter(type %in% c("ORI")),aes(x=start,y=4.5,label=featname,col=ars_col),size=1.7,angle=45,hjust = 0,show.legend=F)+
...
pl_rfd <- lapply(seq_along(chromJunction), function(i) {
	ROI <- suppressWarnings(trim(chromJunction[i]+ovl))
...
pl <- ggplot(toplot_rfd2)+
	...
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=start),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=end),linetype=2,alpha=0.8,col=mypal[9])+
...
pl_init <- lapply(seq_along(chromJunction), function(i) {
	ROI <- suppressWarnings(trim(chromJunction[i]+ovl))
	featROI <- as_tibble(feat[overlapsAny(feat,ROI)]) %>% rename(featname=name) %>% filter(type %in% c("CEN","chromBY")) 
...
pl <- ggplot(toplot_init2)+
	...
	geom_text(data=featROI2,aes(x=start,y=7),label="*",size=6)+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=start),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=end),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("chromBY")),aes(xintercept=start),linetype=4,alpha=0.8)+
	geom_vline(data=featROI%>% filter(type %in% c("chromBY")),aes(xintercept=end),linetype=4,alpha=0.8)+
...
pl_ter <- lapply(seq_along(chromJunction), function(i) {
	ROI <- suppressWarnings(trim(chromJunction[i]+ovl))
	featROI <- as_tibble(feat[overlapsAny(feat,ROI)]) %>% rename(featname=name) %>% filter(type %in% c("CEN","chromBY")) 
	featROI3 <- as_tibble(masked_SY[overlapsAny(masked_SY,ROI)])
...
pl_nano <- lapply(seq_along(chromJunction), function(i) {
	ROI <- suppressWarnings(trim(chromJunction[i]+ovl))
	featROI <- as_tibble(feat[overlapsAny(feat,ROI)]) %>% rename(featname=name) %>% filter(type %in% c("CEN","chromBY")) 
...
pl <- ggplot(toplot_nano2)+
	...
	geom_text(data=featROI2,aes(x=start,y=1.9),label="*",size=6)+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=start),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("CEN")),aes(xintercept=end),linetype=2,alpha=0.8,col=mypal[9])+
	geom_vline(data=featROI%>% filter(type %in% c("chromBY")),aes(xintercept=start),linetype=4,alpha=0.8)+
	geom_vline(data=featROI%>% filter(type %in% c("chromBY")),aes(xintercept=end),linetype=4,alpha=0.8)+
...
themeadj <- theme(
	text = element_text(size =7),  # Taille des polices
...
p_list[[i]] <- pl_rfdpooled/pl_initpooled/pl_terpooled/pl_nanopooled/plot_spacer()/pl_gen+plot_layout(ncol = 1, heights = c(rep(3,4),1.6,3)) & themeadj
}

pp <- wrap_elements(p_list[[8]])+wrap_elements(p_list[[6]])+wrap_elements(p_list[[11]])+wrap_elements(p_list[[4]]) + plot_layout(guides="collect",ncol=2)+plot_annotation(tag_levels="A",title="Figure 5") & theme(text = element_text(face="bold"))
ggsave(plot=pp,paste0("FigArticle/Fig5.pdf"),h=9.05,w=6.88)

pp <- wrap_elements(p_list[[1]])+wrap_elements(p_list[[2]])+wrap_elements(p_list[[3]])+wrap_elements(p_list[[5]]) + plot_layout(guides="collect",ncol=2)+plot_annotation(tag_levels="A",title="Supplementary Figure 5") & theme(text = element_text(face="bold"))
ggsave(plot=pp,paste0("FigArticle/FigS5AD.pdf"),h=9.05,w=6.88)

pp <- wrap_elements(p_list[[7]])+wrap_elements(p_list[[9]])+wrap_elements(p_list[[10]])+wrap_elements(p_list[[12]]) + plot_layout(guides="collect",ncol=2)+plot_annotation(tag_levels=list(c("E","F","G","H")),title="Supplementary Figure 5") & theme(text = element_text(face="bold"))
ggsave(plot=pp,paste0("FigArticle/FigS5EH.pdf"),h=9.05,w=6.88)

pp <- wrap_elements(p_list[[13]])+wrap_elements(p_list[[14]])+wrap_elements(p_list[[15]])+wrap_elements(p_list[[16]]) + plot_layout(guides="collect",ncol=2)+plot_annotation(tag_levels=list(c("I","J","K","L")),title="Supplementary Figure 5") & theme(text = element_text(face="bold"))
ggsave(plot=pp,paste0("FigArticle/FigS5IL.pdf"),h=9.05,w=6.88)

pp <- wrap_elements(p_list[[17]])+plot_spacer()+plot_spacer()+plot_spacer() + plot_layout(guides="collect",ncol=2)+plot_annotation(tag_levels=list(c("M")),title="Supplementary Figure 5") & theme(text = element_text(face="bold"))
ggsave(plot=pp,paste0("FigArticle/FigS5M.pdf"),h=9.05,w=6.88)
