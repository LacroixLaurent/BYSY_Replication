...
ggsave(paste0("FigArticle/Fig",naming[i],".pdf"),h=6.88,w=6.88,device=cairo_pdf)
}
```
...
pl0 <- ggplot(featROI)+
	...
	scale_x_continuous(
		...
		expand=c(0,0))+
	theme(legend.position = "top",axis.ticks.y = element_blank(),axis.text.y = element_blank(),panel.grid.major.y=element_blank(),panel.grid.minor.y=element_blank(),legend.key = element_rect(colour = "black"),legend.key.size = unit(0.2, 'cm'),legend.text=element_text(size=5))+
...
ggsave(plot=pl1,file="FigArticle/FigS2A.pdf",h=2,w=6.88,device=cairo_pdf)
```
